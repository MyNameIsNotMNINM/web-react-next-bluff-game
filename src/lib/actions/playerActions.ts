...
export type PlayerActionType = typeof PlayerActionTypeEnum[keyof typeof PlayerActionTypeEnum];
...
export type PlayerGameActionHandler = {
    ...
    "PLAYER_DATA_UPDATED": (userId: UserId, data: PlayerActionData["PLAYER_DATA_UPDATED"]) => void;
    "BUY_ITEM": (userId: UserId, data: PlayerActionData["BUY_ITEM"]) => void;
    "USE_ITEM": (userId: UserId, data: PlayerActionData["USE_ITEM"]) => void;
};

export type PlayerLobbyActionHandler = {
    "CHAT_MESSAGE": (userId: UserId, data: PlayerActionData["CHAT_MESSAGE"]) => void;
...
export type PlayerActionGameMessage = Exclude<PlayerActionMessage, { type:  keyof PlayerLobbyActionHandler }>;
...
const PlayCardsSchema = z.object({
    ...
    payload: z.object({
        cards: z.array(z.union([z.literal('A'), z.literal('K'), z.literal('Q'), z.literal('JK')])).min(1).max(3),
    }),
});
